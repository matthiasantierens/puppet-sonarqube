  ...
  on_supported_os.each do |os, _facts|
    context "on #{os}" do
      ...
      context 'when installing a plugin from sonarsource' do
        ...
        it { is_expected.to compile.with_all_deps }
        it { is_expected.to contain_archive("download sonarqube plugin #{plugin_name}-#{plugin_version}.jar from SonarSource").with_source("https://binaries.sonarsource.com/Distribution/#{plugin_name}/#{plugin_name}-#{plugin_version}.jar") }
      ...
      context 'when installing a plugin from github' do
        ...
        it { is_expected.to compile.with_all_deps }
        it { is_expected.to contain_archive("download sonarqube plugin #{plugin_name}-#{plugin_version}.jar from GitHub").with_source("https://github.com/#{ghid}/releases/download/#{plugin_version}/#{plugin_name}-#{plugin_version}.jar") }
        it { is_expected.to contain_file("/var/local/sonar/extensions/plugins/#{plugin_name}-#{plugin_version}.jar").with_owner('sonar') }
        it { is_expected.to contain_exec("remove old versions of #{plugin_name}") }
      end
